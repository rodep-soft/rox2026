...
: lc::LifecycleNode("socket_can_sender_node", options)
{
  ...
  enable_frame_loopback_ = this->declare_parameter("enable_frame_loopback", true);
...
LNI::CallbackReturn SocketCanSenderNode::on_configure(const lc::State & state)
{
  ...
  try {
    sender_ = std::make_unique<SocketCanSender>(
      interface_, enable_fd_, CanId{}, enable_frame_loopback_);
  ...
  auto can_tx_qos = rclcpp::QoS(rclcpp::KeepLast(500))
    .reliable()
    .durability_volatile();
