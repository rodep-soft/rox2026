...
SocketCanSender::SocketCanSender(
  ...
  const bool enable_frame_loopback)
: m_enable_fd(enable_fd),
  m_file_descriptor{bind_can_socket(interface, m_enable_fd)},
  m_default_id{default_id}
{
  const int loopback = enable_frame_loopback ? 1 : 0;
  if (setsockopt(
      m_file_descriptor, SOL_CAN_RAW, CAN_RAW_LOOPBACK,
      &loopback, sizeof(loopback)) < 0)
  {
    throw std::runtime_error{
            "Failed to configure CAN frame loopback: " + std::string{strerror(errno)}};
  }
}
