...
private:
    ...
    {                                                  // 制御コマンドの受け取り
        ...
        if (target_state_ == State::STOP)
        { // 停止コマンドは即時適用
            now_state_ = State::STOP;
        }
        else if (target_state_ == State::LOAD && now_state_ == State::STOP)
        { // 停止状態から発射命令が来た場合は装填状態に遷移
    ...
    { // モーター制御コールバック
        ...
        RCLCPP_INFO(this->get_logger(), "now_state: %d, target_state: %d, speed: %f, limit_sw: %d", now_state_, target_state_, speed.data, limit_sw_);
