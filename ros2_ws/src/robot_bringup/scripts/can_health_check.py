...
def check_can_interface_stats():
    ...
    try:
        ...
        if "ERROR_PASSIVE" in bus_state or "ERROR-PASSIVE" in bus_state:
            print(
                f"  State          : {RED}{BOLD}ERROR-PASSIVE  ← CAN バスにエラー多発中!{RESET}"
            )
        ...
        print(
            f"  Bitrate        : {int(bitrate)//1000 if bitrate.isdigit() else bitrate} kbps"
        ...
        if berr_match:
            ...
            if tec > 96 or rec > 96:
                print(
                    f"  {RED}⚠ エラーカウンタ高値: CAN フレームに ACK が返っていないか、"
...
def analyze_can_bus_traffic():
    ...
    if error_frames > 0:
        print(
            f"  {RED}Error Frames   : {error_frames}  ← CAN バス上でエラーフレームを検出!{RESET}"
        )
    ...
    for motor_id, name in EDULITE_MOTOR_MAP.items():
        ...
        status = (
            f"{GREEN}ONLINE ({node_fps:5.1f} Hz){RESET}"
            if count > 0
            else f"{RED}NO RESPONSE{RESET}"
        )
    ...
    if error_frames > 5:
        ...
        print(
            f"    → EduLite が ACK を返さない / フォーマット不一致 / 配線問題の可能性"
        )
    ...
    if edulite_online == 0 and vesc_online:
        ...
        print(
            "    → Extended フレーム(29-bit)として送信できているか `candump can0` で確認。"
        )
    elif edulite_online > 0 and edulite_online < len(EDULITE_MOTOR_MAP):
        offline = [
            name.strip()
            for mid, name in EDULITE_MOTOR_MAP.items()
