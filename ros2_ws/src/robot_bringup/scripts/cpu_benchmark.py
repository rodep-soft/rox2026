...
def main():
    ...
    interval = 1.0
    print(f"📊 [RDK X5 Benchmark] Starting CPU & Memory benchmark for {duration} seconds...")
    print("=" * 75)
    print(f"{'Time':<8} | {'Total CPU%':<10} | {'RAM Used (MB)':<14} | {'Top Active ROS Node / Process'}")
    print("-" * 75)
    ...
    for i in range(duration):
        ...
        for p in psutil.process_iter(['name', 'cmdline', 'cpu_percent']):
            try:
                ...
                if 'ros' in cmd.lower() or 'apriltag' in cmd.lower() or 'mipi' in cmd.lower() or 'v4l2' in cmd.lower() or 'ekf' in cmd.lower() or 'mecanum' in cmd.lower() or 'bno' in cmd.lower():
        ...
        print(f"{i+1:02d}s     | {total_cpu:6.1f}%    | {ram_mb:8.1f} MB     | {top_proc_str}")
