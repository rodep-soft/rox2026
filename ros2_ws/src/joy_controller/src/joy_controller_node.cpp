...
JoyControllerNode::JoyControllerNode()
  ...
  if (joy_qos_depth_ <= 0) {
    RCLCPP_WARN(
        get_logger(),
        "joy_qos_depth must be positive. Using the default value of 1.");
    joy_qos_depth_ = 1;
  }
  if (command_qos_depth_ <= 0) {
    RCLCPP_WARN(
        get_logger(),
        "command_qos_depth must be positive. Using the default value of 1.");
    command_qos_depth_ = 1;
  }
  if (joy_timeout_ms_ <= 0) {
    RCLCPP_WARN(
        get_logger(),
        "joy_timeout_ms must be positive. Using the default value of 200 ms.");
    joy_timeout_ms_ = 200;
  }
  if (control_period_ms_ <= 0) {
    RCLCPP_WARN(get_logger(),
                "control_period_ms must be positive. Using the default value "
                "of 10 ms.");
  ...
  control_timer_ = create_wall_timer(
      ...
      std::bind(&JoyControllerNode::control_timer_callback, this));

  RCLCPP_INFO(get_logger(),
              "Subscribing to %s and publishing mechanism commands",
              joy_topic_.c_str());
}
...
void JoyControllerNode::get_parameters() {
  ...
  get_parameter("control_period_ms", control_period_ms_);
  ...
  get_parameter("dribble_mode_is_enable_button",
                dribble_mode_is_enable_button_);
  get_parameter("emergency_stop_is_enable_button",
                emergency_stop_is_enable_button_);
  get_parameter("emergency_stop_button_on", emergency_stop_button_on_);
  get_parameter("dribble_position_is_enable_button",
                dribble_position_is_enable_button_);
  get_parameter("dribble_position_dribble_button",
                dribble_position_dribble_button_);
  get_parameter("dribble_position_shoot_button",
                dribble_position_shoot_button_);
...
double JoyControllerNode::axis_value(const sensor_msgs::msg::Joy& msg,
  ...
  return index >= 0 && static_cast<std::size_t>(index) < msg.axes.size()
             ? msg.axes[static_cast<std::size_t>(index)]
             : 0.0;
}
...
double JoyControllerNode::apply_axis_limits(double value, double minimum,
                                            double maximum) {
...
void JoyControllerNode::control_timer_callback() {
  if (!joy_received_) {
    return;
  }

  const auto elapsed =
      std::chrono::steady_clock::now() - last_joy_received_time_;
  ...
  process_joy_message(latest_joy_msg_);
}

void JoyControllerNode::update_chord_inputs(const sensor_msgs::msg::Joy& msg) {
  const bool intake_is_enable_button =
      button_pressed(msg, intake_is_enable_button_);
  const bool intake_button_on = button_pressed(msg, intake_button_on_);
  const bool spring_fire_is_enable_button =
      button_pressed(msg, spring_fire_is_enable_button_);
  const bool spring_fire_button_on =
      button_pressed(msg, spring_fire_button_on_);
  const bool belt_mode_is_enable_button =
      button_pressed(msg, belt_mode_is_enable_button_);
  const bool dribble_mode_is_enable_button =
      button_pressed(msg, dribble_mode_is_enable_button_);
  const bool emergency_stop_is_enable_button =
      button_pressed(msg, emergency_stop_is_enable_button_);
  const bool emergency_stop_button_on =
      button_pressed(msg, emergency_stop_button_on_);
...
bool JoyControllerNode::handle_emergency_stop() {
  ...
  if (emergency_stop_latched_) {
    ...
    RCLCPP_WARN_THROTTLE(
        ...
        "Emergency stop mode is active. Sending stop commands.");
    publish_stop_commands();
    return true;
  }
...
void JoyControllerNode::process_joy_message(
  ...
  if (dribble_position_dribble_chord_on_ &&
      !pre_dribble_position_dribble_chord_on_) {
    publish_dribble_position_mode(DribblePositionMode::DRIBBLE);
  }
  if (dribble_position_shoot_chord_on_ &&
      !pre_dribble_position_shoot_chord_on_) {
    publish_dribble_position_mode(DribblePositionMode::SHOOT);
  }

  const double linear_x =
      apply_axis_deadzone(axis_value(joy_msg, left_stick_y_axis_));
  const double linear_y =
      apply_axis_deadzone(axis_value(joy_msg, left_stick_x_axis_));
  const double angular_z =
      apply_axis_deadzone(axis_value(joy_msg, right_stick_x_axis_));

  // スティック入力に制限値とスケールをかけて、mecanum_controller向けの速度指令にする。
  cmd_vel_.linear.x =
      apply_axis_limits(linear_x, min_linear_x_, max_linear_x_) *
      linear_x_scale_;
  cmd_vel_.linear.y =
      apply_axis_limits(linear_y, min_linear_y_, max_linear_y_) *
      linear_y_scale_;
  cmd_vel_.angular.z =
      apply_axis_limits(angular_z, min_angular_z_, max_angular_z_) *
      angular_z_scale_;
