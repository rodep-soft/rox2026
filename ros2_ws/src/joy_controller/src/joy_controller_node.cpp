...
void JoyControllerNode::declare_parameters()
{
  ...
  declare_parameter<std::string>(
    "dribble_position_mode_topic",
    "/dribble/position_mode");
  declare_parameter<std::string>(
    "intake_shoot_request_topic",
...
uint8_t JoyControllerNode::increment_mode(
  uint8_t mode,
...
void JoyControllerNode::update_chord_inputs(
  const sensor_msgs::msg::Joy & msg)
{
  ...
  const sensor_msgs::msg::Joy & joy_msg)
{
  ...
  if (belt_mode_up_chord_on_ && !pre_belt_mode_up_chord_on_) {
    belt_rpm_mode_ = increment_mode(
      belt_rpm_mode_, static_cast<uint8_t>(BeltRpmMode::LEVEL_3));
  }
  ...
  if (belt_ready_ && intake_shoot_pending_) {
    intake_shoot_pending_ = false;
    publish_intake_shoot_request();
  }
