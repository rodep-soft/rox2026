...
class JoyControllerNode : public rclcpp::Node {
 ...
 private:
  enum class BeltRpmMode : uint8_t { STOP = 1, LEVEL_1, LEVEL_2, LEVEL_3 };
  ...
  std::string mecanum_cmd_vel_topic_, spring_fire_request_topic_,
      belt_mode_topic_, dribble_enabled_topic_, emergency_stop_topic_,
      dribble_position_mode_topic_, intake_shoot_request_topic_,
      belt_ready_topic_;
  int joy_qos_depth_{1};
  int command_qos_depth_{1};
  int joy_timeout_ms_{200};
  int state_publish_period_ms_{20};
  double linear_x_scale_{1.0}, linear_y_scale_{1.0}, angular_z_scale_{1.0};
  ...
  rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr belt_ready_subscription_;
