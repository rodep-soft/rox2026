...
void MecanumControllerNode::create_interfaces() {
  cmd_vel_sub_ = create_subscription<geometry_msgs::msg::Twist>(
      ...
      std::bind(&MecanumControllerNode::cmd_vel_callback, this,
                std::placeholders::_1));
  const auto state_qos = rclcpp::QoS(1).reliable().transient_local();
  operation_mode_sub_ = create_subscription<std_msgs::msg::UInt8>(
      operation_mode_topic_, state_qos,
      std::bind(&MecanumControllerNode::operation_mode_callback, this,
                std::placeholders::_1));
  emergency_stop_sub_ = create_subscription<std_msgs::msg::Bool>(
      emergency_stop_topic_, state_qos,
      std::bind(&MecanumControllerNode::emergency_stop_callback, this,
                std::placeholders::_1));
...
void MecanumControllerNode::declare_parameters() {
  ...
  declare_parameter<std::vector<double>>("velocity_corrections",
                                         {1.0, 1.0, 1.0, 1.0});
  ...
  declare_parameter<std::string>("front_left_velocity_topic",
                                 "/mecanum/front_left/vel_command");
  declare_parameter<std::string>("front_right_velocity_topic",
                                 "/mecanum/front_right/vel_command");
  declare_parameter<std::string>("rear_left_velocity_topic",
                                 "/mecanum/rear_left/vel_command");
  declare_parameter<std::string>("rear_right_velocity_topic",
...
void MecanumControllerNode::publish_wheel_commands() {
  ...
  if (emergency_stop_active_ || operation_mode_ == OperationMode::STOP) {
    ...
    wz_ = 0.0;
             ...
             operation_mode_ == OperationMode::GAME2_MODE) {
    vx_ = 0.0;
    vy_ = 0.0;
  }
