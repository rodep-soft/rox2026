...
void BeltDribbleController::validate_parameters()
{
  ...
  if (!std::isfinite(ready_hold_sec_) || ready_hold_sec_ < 0.0) {
    RCLCPP_ERROR(
      get_logger(),
      "ready_hold_sec must be finite and zero or greater: %.3f",
      ready_hold_sec_);
    configuration_valid_ = false;
  }
...
void BeltDribbleController::create_interfaces()
{
  ...
  // /operation_mode: joy_controllerから受ける機構操作モード。停止・shot
  // cycle可否を決める。
  ...
  // /belt/mode, /dribble/enabled, /shot_cycle/request:
  // joy_controllerからの機構操作要求。
  ...
  // 各/current/rpm: hardware_driverから受ける3モータの実RPM。shot
  // cycle開始判定に使う。
  ...
  // 各/target/rpm: hardware_driverへ送る3モータの目標RPM。
  ...
  // /shot_cycle/start:
  // RPMが到達しているときにdribble_position_controllerへ送る開始要求。
  shot_cycle_start_publisher_ =
    create_publisher<std_msgs::msg::Bool>("/shot_cycle/start", command_qos);
}
...
{
  // /operation_mode受信時にモードを更新し、変更があればshot
  // cycle準備状態を解除する。publishはしない。
  ...
  const std_msgs::msg::Bool::SharedPtr msg)
{
  if (msg->data != dribble_enabled_) {
    ...
    reset_shoot_ready();
    RCLCPP_INFO(
      get_logger(), "Dribble %s: target=%d RPM",
  ...
  const std_msgs::msg::Int16::SharedPtr msg)
{
  ...
  if (was_timed_out) {
    RCLCPP_INFO(
      get_logger(), "Underbelt feedback resumed: %d RPM",
  ...
  const std_msgs::msg::Int16::SharedPtr msg)
{
  ...
  if (was_timed_out) {
    RCLCPP_INFO(
      get_logger(), "Upperbelt feedback resumed: %d RPM",
  ...
  const std_msgs::msg::Int16::SharedPtr msg)
{
  ...
  if (was_timed_out) {
    RCLCPP_INFO(
      get_logger(), "Dribble feedback resumed: %d RPM",
      dribble_current_rpm_);
  }
}
...
{
  // 設定周期で各/target/rpmをpublishし、最新feedbackからshot
  // cycle開始可能状態を更新する。
    ...
    current_dribble_target != last_logged_dribble_target_rpm_)
  {
    RCLCPP_INFO(
      ...
      dribble_enabled_ ? "ON" : "OFF", emergency_stop_active_ ? "ON" : "OFF",
      current_belt_target, current_belt_target, current_dribble_target);
  ...
  shoot_ready_ = update_shoot_ready(
    current_belt_target, current_dribble_target,
    current_time);
}
...
bool BeltDribbleController::update_shoot_ready(
  int current_belt_target,
  int current_dribble_target,
  ...
  const rclcpp::Time & current_time)
{
  const auto update_timeout =
      ...
      const rclcpp::Time & received_at, bool & timed_out)
    {
      ...
      timed_out = true;
      RCLCPP_WARN(
        get_logger(), "%s feedback timed out after %.3f seconds.",
        motor_name, (current_time - received_at).seconds());
    };
  update_timeout(
    "Underbelt", underbelt_feedback_received_,
    underbelt_feedback_received_at_,
    underbelt_feedback_timed_out_);
  update_timeout(
    "Upperbelt", upperbelt_feedback_received_,
    upperbelt_feedback_received_at_,
    upperbelt_feedback_timed_out_);
  update_timeout(
    "Dribble", dribble_feedback_received_,
    dribble_feedback_received_at_, dribble_feedback_timed_out_);
}

const char * BeltDribbleController::operation_mode_name(OperationMode mode) const
{
  switch (mode) {
