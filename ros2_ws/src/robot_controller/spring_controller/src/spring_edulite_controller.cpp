...
: Node("spring_controller_node")
{
  ...
  if (!std::isfinite(fire_duration_sec_) || fire_duration_sec_ <= 0.0) {
    RCLCPP_ERROR(
      get_logger(),
      "fire_duration_sec must be finite and greater than zero: %.3f",
      fire_duration_sec_);
    is_configuration_valid_ = false;
  }
  if (!std::isfinite(load_timeout_sec_) || load_timeout_sec_ <= 0.0) {
    RCLCPP_ERROR(
      get_logger(),
      "load_timeout_sec must be finite and greater than zero: %.3f",
      load_timeout_sec_);
    is_configuration_valid_ = false;
  }
  if (command_period_ms_ <= 0) {
    RCLCPP_ERROR(
      get_logger(),
      "command_period_ms must be greater than zero: %d",
      command_period_ms_);
    ...
    std::abs(fire_velocity_rad_s_) > edulite_velocity_limit_rad_s)
  {
    RCLCPP_ERROR(
      get_logger(),
      "Spring velocity magnitude must not exceed 50 rad/s: "
      "loading=%.6f rad/s, fire=%.6f rad/s",
      loading_velocity_rad_s_, fire_velocity_rad_s_);
    is_configuration_valid_ = false;
  }
  if (!is_configuration_valid_) {
    RCLCPP_ERROR(
      get_logger(),
      "Spring controller configuration is invalid. Velocity command "
      "is fixed at 0 rad/s.");
  }
  ...
  fire_request_sub_ = create_subscription<std_msgs::msg::Bool>(
    ...
    std::bind(
      ...
      std::placeholders::_1));
  // /emergency_stop: joy_controllerから受ける非常停止状態。
  emergency_stop_sub_ = create_subscription<std_msgs::msg::Bool>(
    ...
    std::bind(
      ...
      std::placeholders::_1));
  // /limit_switchs:
  // hardware_driverから受ける装填完了を示すリミットスイッチビット列。
  limit_switch_sub_ = create_subscription<std_msgs::msg::UInt8>(
    ...
    std::bind(
      ...
      std::placeholders::_1));
  // /spring/vel_command: hardware_driverへ送るばねモータの目標角速度 [rad/s]。
...
{
  // /spring/fire_requestの立上り受信時に、READYかつ許可状態なら発射待ちへ遷移する。
...
void SpringEduliteController::emergency_stop_callback(
  const std_msgs::msg::Bool::SharedPtr msg)
{
  // /emergency_stop受信時に発射を中断して停止準備へ遷移する。速度指令のpublishはtimerが行う。
  if (msg->data != emergency_stop_active_) {
    if (msg->data) {
      RCLCPP_WARN(
        get_logger(), "Emergency stop activated: state=%s.",
  ...
  if (emergency_stop_active_) {
    ...
    if (now_state_ == State::LOAD) {
      RCLCPP_WARN(
        get_logger(),
        "Emergency stop is active, but the current controller "
        "behavior continues loading at %.3f rad/s.",
        loading_velocity_rad_s_);
    }
  }
}
...
void SpringEduliteController::limit_switch_callback(
  const std_msgs::msg::UInt8::SharedPtr msg)
{
  // /limit_switchs受信時に装填完了状態を更新する。publishはしない。
...
{
  // 設定周期でLOAD/READY/FIREを遷移し、/spring/vel_commandへ現在状態の目標速度をpublishする。
  ...
  if (!spring_fire_allowed()) {
    ...
    if (now_state_ == State::FIRE) {
      RCLCPP_WARN(
        get_logger(),
        "Spring fire interrupted while command was being sent.");
      start_loading();
    }
  }
  switch (now_state_) {
...
void SpringEduliteController::start_loading()
{
  ...
  if (!was_loading) {
    RCLCPP_INFO(
      get_logger(), "Spring loading started: velocity=%.3f rad/s.",
      loading_velocity_rad_s_);
  }
}
