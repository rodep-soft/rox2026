...
public:
  ...
  : Node("field_visualization_node", options)
  {
    ...
    RCLCPP_INFO(get_logger(), "FieldVisualizationNode: Publishing field layout with 100%% SDF Mesh Alignment and correct Left-Right panel mapping");
  }
...
private:
  void publish_field_markers()
  {
    ...
    // 6. 全27枚の公式 AprilTag (SDF CAD メッシュのテクスチャ面座標に 100% 一致)
    {
      struct TagData {
        int id;
      ...
      const std::vector<TagData> all_tags = {
        ...
        // コート内側（ロボット視点）から見て:
        // 左列 (X = -3.64m): 14 (上), 17 (中), 20 (下)
        // 中列 (X = -3.23m): 15 (上), 18 (中), 21 (下)
        // 右列 (X = -2.82m): 16 (上), 19 (中), 22 (下)
