...
public:
  ...
  : Node("field_visualization_node", options)
  {
    ...
    RCLCPP_INFO(get_logger(), "FieldVisualizationNode: Publishing field layout with 100%% Rulebook-matched 12/13 and GAME3 Goal tags");
  }
...
private:
  void publish_field_markers()
  {
    ...
    // 6. 全27枚の公式 AprilTag (図面 2.3 完全一致)
    {
      struct TagData {
        int id;
      ...
      const std::vector<TagData> all_tags = {
        ...
        // センターラインポール (図面通り: 12が上 Y=2.45m, 13が下 Y=0.65m)
        {12, -0.020,  2.450, 0.302,  3.1416},   // 自陣西向き (上側ポール)
        {13, -0.020,  0.650, 0.302,  3.1416},   // 自陣西向き (下側ポール)

        // GAME2 3x3 シュートパネル (自陣下側 X = -3.23m, Y = -5.525m)
        // 上段 (Z = 0.97m): 14(左 X=-3.64), 15(中 X=-3.23), 16(右 X=-2.82)
        // 中段 (Z = 0.51m): 17(左 X=-3.64), 18(中 X=-3.23), 19(右 X=-2.82)
        // 下段 (Z = 0.05m): 20(左 X=-3.64), 21(中 X=-3.23), 22(右 X=-2.82)
        ...
        // 正面から見て:
        // 左上 (X = -0.75m, Z = 0.85m): 23
        // 右上 (X = +0.95m, Z = 0.85m): 24
        // 左下 (X = -0.75m, Z = 0.12m): 25
        // 右下 (X = +0.95m, Z = 0.12m): 26
