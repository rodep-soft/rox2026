...
public:
  ...
  : Node("field_visualization_node", options)
  {
    ...
    RCLCPP_INFO(get_logger(), "FieldVisualizationNode: Publishing verified field layout and AprilTags matching Rulebook 2.3");
  }
...
private:
  void publish_field_markers()
  {
    ...
    // 5. GAME1 DFゲート構造体 (SDF CAD メッシュに完全一致)
    {
      struct DFGateDef {
    ...
    // 8. 全27枚の公式 AprilTag (ルールブック「2.3 2次元コード」完全準拠)
    {
      struct TagData {
        int id;
      ...
      const std::vector<TagData> all_tags = {
        // --- 四隅・外壁 (自陣 SIDE A) ---
        {0,  -5.920,  5.490, 0.300,  1.5708},   // 上壁 (北向き)
        {1,  -6.500,  4.920, 0.300,  3.1416},   // 左上壁 (西向き)
        {2,  -6.490, -4.920, 0.300,  3.1416},   // 左下壁 (西向き)
        {3,  -5.920, -5.500, 0.300, -1.5708},   // 下壁 (南向き)
        // 敵陣 (SIDE B):
        ...
        // --- 中段縦向きゲート (自陣 SIDE A: I) ---
        // 上支柱: 9, 下支柱: 11, 左バー: 8, 右バー: 10
        ...
        // --- センターラインポール ---
        {12, -0.020,  2.450, 0.400,  3.1416},   // 上側ポール (北側)
        {13, -0.020,  0.650, 0.400,  3.1416},   // 下側ポール (南側)

        // --- GAME2 3x3 シュートパネル (図面通り 14..22) ---
        // 上段 (Z = 1.19m): 14 (左), 15 (中), 16 (右)
        {14, -3.640, -5.525, 1.190,  1.5708},
        {15, -3.230, -5.525, 1.190,  1.5708},
        {16, -2.820, -5.525, 1.190,  1.5708},
        // 中段 (Z = 0.73m): 17 (左), 18 (中), 19 (右)
        {17, -3.640, -5.525, 0.730,  1.5708},
        {18, -3.230, -5.525, 0.730,  1.5708},
        {19, -2.820, -5.525, 0.730,  1.5708},
        // 下段 (Z = 0.27m): 20 (左), 21 (中), 22 (右)
        {20, -3.640, -5.525, 0.270,  1.5708},
        {21, -3.230, -5.525, 0.270,  1.5708},
        {22, -2.820, -5.525, 0.270,  1.5708},

        // --- GAME3 ゴールエリア (図面通り 23..26) ---
        // 上段 (Z = 0.85m): 23 (左上), 24 (右上)
        {23, -0.750, -5.510, 0.850,  1.5708},
        {24,  0.950, -5.510, 0.850,  1.5708},
        // 下段 (Z = 0.12m): 25 (左下), 26 (右下)
