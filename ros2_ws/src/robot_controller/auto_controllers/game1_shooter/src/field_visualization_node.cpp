...
public:
  ...
  : Node("field_visualization_node", options)
  {
    ...
    RCLCPP_INFO(get_logger(), "FieldVisualizationNode: Publishing field layout and elevated panels to /field/markers");
  }
...
private:
  void publish_field_markers()
  {
    ...
    // 7. GAME2 3x3 シュートパネル構造体 (下段 Z=0.27m, 中段 Z=0.73m, 上段 Z=1.19m で地面から完全浮上)
    {
      const std::vector<double> g2_xs = {-3.43, 3.43};
      for (const double g2_x : g2_xs) {
        for (int r = 0; r < 3; ++r) {
          for (int c = 0; c < 3; ++c) {
            ...
            panel.pose.position.z = 0.27 + r * 0.46; // 下段 Z=0.27m (底辺 Z=0.09m で地面から浮上)
    ...
    // 8. 全27枚の公式 AprilTag (前repo tag_localizer 完全一致座標)
    {
      struct TagData {
        int id;
      ...
      const std::vector<TagData> all_tags = {
        ...
        {1,  -6.020, -5.495, 0.420,  1.571},
        {2,  -6.500,  4.920, 0.320,  1.571},
        {3,  -6.020,  5.495, 0.420, -1.571},

        // GAME1 縦向きDFゲート
        {4,  -4.505,  2.165, 0.922,  1.571},
        {5,  -4.445,  2.165, 0.922, -1.571},
        {6,  -4.475,  2.495, 0.400,  1.571},
        {7,  -4.475,  1.835, 0.400, -1.571},

        // GAME1 横向きDFゲート
        {8,  -3.165,  3.905, 0.922,  1.571},
        {9,  -3.495,  3.875, 0.400,  3.142},
        {10, -2.835,  3.875, 0.400,  0.000},
        {11, -3.165,  3.845, 0.922, -1.571},

        // センターラインポール
        {12,  0.015,  2.550, 0.422,  3.142},
        {13,  0.015,  0.750, 0.422,  3.142},

        // GAME2 3x3 シュートパネル (地面に埋まらない正しい高さ: 0.27m / 0.73m / 1.19m)
        {14, -3.430, -5.525, 0.270,  1.571},
        {15, -3.020, -5.525, 0.270,  1.571},
        {16, -3.840, -5.525, 0.730,  1.571},
        {17, -3.430, -5.525, 1.190,  1.571},
        {18, -3.430, -5.525, 0.730,  1.571},
        {19, -3.020, -5.525, 1.190,  1.571},
        {20, -3.840, -5.525, 1.190,  1.571},
        {21, -3.020, -5.525, 0.730,  1.571},
        {22, -3.840, -5.525, 0.270,  1.571},
