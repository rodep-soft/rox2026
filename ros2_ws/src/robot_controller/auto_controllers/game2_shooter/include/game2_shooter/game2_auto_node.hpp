...
public:
  ...
  ~Game2AutoNode() override = default;
...
private:
  ...
  double camera_fx_{723.47};
  double camera_fy_{723.47};
  double camera_cx_{746.35};
  double camera_cy_{578.43};

  // Tolerances & Timings
  double dist_tolerance_{0.05};
  double tag_pitch_x_{0.40};
  double tag_pitch_y_{0.43};
  ...
  double belt_spinup_duration_{0.5};
  double tag_lost_timeout_{0.5};
  double aligning_timeout_{10.0};
  double shooting_timeout_{3.0};
  double result_wait_duration_{1.0};
  int max_shots_per_panel_{0};
  int max_total_balls_{15};
  bool enable_ball_limit_{false};
