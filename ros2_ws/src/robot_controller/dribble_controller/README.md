...
| sub | `/dribble/command_position` | `DRIBBLE``OPEN``FEED``ZERO_POSITION` 姿勢 |
...
- `ZERO_POSITION`：手動操作用の0 rad姿勢（角度: `zero_position_rad`）。
...
- `dribble_position_rad``open_position_rad``zero_position_rad``feed_position_rad`
- feed_duration_sec
