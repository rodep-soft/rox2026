...
private:
  ...
  static constexpr uint8_t low_mode_{1};
  ...
  void timer_callback();
  double target_velocity_from_mode(uint8_t mode);

  bool is_configuration_valid_{true};
  bool stop_requested_{false};
  uint8_t dribble_mode_{stop_mode_};
  double current_velocity_rad_s_{0.0};
  double low_velocity_rad_s_{30.0};
  double high_velocity_rad_s_{60.0};
  double stop_deceleration_rad_s2_{20.0};
  ...
  rclcpp::Publisher<std_msgs::msg::Float32>::SharedPtr velocity_command_pub_;
