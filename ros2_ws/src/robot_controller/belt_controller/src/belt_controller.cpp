...
: Node("belt_controller_node")
{
  ...
  if (ready_tolerance_rpm_ < 0) {ready_tolerance_rpm_ = 100;}
  if (ready_hold_sec_ < 0.0) {ready_hold_sec_ = 0.1;}
  ...
  underbelt_feedback_sub_ = create_subscription<std_msgs::msg::Int16>(
    ...
    std::bind(&BeltControllerNode::underbelt_feedback_callback, this, std::placeholders::_1));
  upperbelt_feedback_sub_ = create_subscription<std_msgs::msg::Int16>(
    upperbelt_current_rpm_topic_, rclcpp::QoS(qos_depth_),
    std::bind(&BeltControllerNode::upperbelt_feedback_callback, this, std::placeholders::_1));
...
void BeltControllerNode::declare_parameters()
{
  ...
  declare_parameter<std::string>("underbelt_current_rpm_topic", "/underbelt/current/rpm");
  declare_parameter<std::string>("upperbelt_current_rpm_topic", "/upperbelt/current/rpm");
...
void BeltControllerNode::underbelt_feedback_callback(const std_msgs::msg::Int16::SharedPtr msg)
{
  underbelt_current_rpm_ = msg->data;
  underbelt_feedback_received_ = true;
}

void BeltControllerNode::upperbelt_feedback_callback(const std_msgs::msg::Int16::SharedPtr msg)
{
...
bool BeltControllerNode::is_belt_ready(int target_rpm, const rclcpp::Time & current_time)
{
  const bool within_tolerance = target_rpm != stop_rpm_ && underbelt_feedback_received_ &&
    upperbelt_feedback_received_ && std::abs(underbelt_current_rpm_ - target_rpm) <= ready_tolerance_rpm_ &&
    std::abs(upperbelt_current_rpm_ - target_rpm) <= ready_tolerance_rpm_;
  if (!within_tolerance) {
    ready_since_ = rclcpp::Time(0, 0, get_clock()->get_clock_type());
    return false;
  }
  if (ready_since_.nanoseconds() == 0) {ready_since_ = current_time;}
