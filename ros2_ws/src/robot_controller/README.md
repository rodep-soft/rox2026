...
`belt_mode`は`STOP (0)`、`LEVEL_1 (1)`、`LEVEL_2 (2)`、`LEVEL_3 (3)`の4段階です。ベルト有効入力が`false`または`belt_mode`が`STOP`の場合は、`0 RPM`をpublishします。範囲外のmodeを受けた場合も、安全側として`0 RPM`をpublishします。

`level_1_rpm`〜`level_3_rpm`と指令周期は`robot_bringup/config/belt_controller.yaml`で設定できます。停止時の目標回転数は安全のためコード側で`0 RPM`固定です。起動には`robot_bringup/launch/belt_controller.launch.py`を使います。
