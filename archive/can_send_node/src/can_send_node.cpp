#include "can_send_node/can_send_node.hpp"
...
CanSendNode::CanSendNode()
: Node("can_send_node"), can_socket_(-1)
{
  DeclareParameters();
  LoadParameters();
  OpenCanSocket();
  ...
  RCLCPP_INFO(
    this->get_logger(),
    "CanSendNode started. interface=%s, channels=%zu",
...
void CanSendNode::DeclareParameters()
{
...
void CanSendNode::LoadParameters()
{
...
bool CanSendNode::GetChannelEnabled(const std::string & channel_name, bool default_value)
{
...
CanSendNode::ChannelConfig CanSendNode::LoadChannelConfig(
...
void CanSendNode::OpenCanSocket()
{
...
void CanSendNode::CloseCanSocket()
{
  if (can_socket_ >= 0) {
    close(can_socket_);
    can_socket_ = -1;
...
void CanSendNode::SetupSubscriptions()
{
