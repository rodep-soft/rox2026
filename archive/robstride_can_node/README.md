...
ノード起動から100ms後、以下を一度だけ送る(`startup_timer_`)。

position modeではRobStride 05マニュアルのPP位置モード手順に合わせ、以下の順に送る:

1. `set_zero_on_startup: true` なら現在位置を機械ゼロに設定(Communication Type 6)
2. `run_mode=1` を書き込み(PP位置モード)
3. `enable_on_startup: true` ならモーター有効化コマンド(Communication Type 3)
4. `position_speed` が0.0以上なら `vel_max` を書き込み
5. `position_acceleration` が0.0以上なら `acc_set` を書き込み
6. `limit_torque` / `limit_cur` / `limit_spd` のうち値が設定されているもの(0.0以上)だけを書き込み
7. 起動時の初期 `loc_ref` を送信

velocity modeでは `run_mode=2`、`limit_*`、必要なら有効化コマンドの順に送る。

`run_mode`や`limit_*`はモーター側が設定を保持するため、これらは起動時の1回だけで十分。Type 3(有効化)はType 18の書き込みを「反映」させるものではなく、モーターの制御ループ・出力そのものを有効にするスイッチ。
...
| `set_zero_on_startup` | position mode起動時に現在位置を機械ゼロに設定するか |
| `position_speed` | PP位置モード速度。負値(デフォルト)は未指定で変更しない |
| `position_acceleration` | PP位置モード加速度。負値(デフォルト)は未指定で変更しない |
...
| `0x700B` | limit_torque | float | トルク制限[N・m] (0~5.5) |
