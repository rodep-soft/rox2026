...
if ! command -v intercept >/dev/null 2>&1 || ! command -v udevmon >/dev/null 2>&1; then
  # Use a distro package when available; otherwise build upstream.
  if apt-cache show interception-tools >/dev/null 2>&1; then
    apt-get install -y interception-tools
    # Debian/Raspberry Pi OS trixie renames upstream `intercept` to
    # `interception`; the compat package restores the upstream command name.
    if ! command -v intercept >/dev/null 2>&1 && apt-cache show interception-tools-compat >/dev/null 2>&1; then
