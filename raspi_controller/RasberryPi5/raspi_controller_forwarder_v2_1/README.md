...
## v2.1 low-latency RFCOMM update

The v2.1 sender keeps protocol v2 compatible with the RDK v2 bridge, but avoids
RFCOMM backlog under heavy DualSense stick traffic.

Defaults:
- AXIS_RATE=100 Hz: coalesce EV_ABS to the latest value at <=100 Hz.
- ABS_MIN_DELTA=1: suppress one-count analog jitter; D-pad HAT axes are exempt.
- EV_KEY transitions are flushed immediately at the next SYN_REPORT.
- A full evdev report is transmitted as one UDP datagram / RFCOMM write batch.
- BT_MAX_LATENCY=0.10 s: reset the local Bluetooth writer rather than replay stale actions.
- Recommended BT_QUEUE_SIZE=16.

For an existing installation, replacing only `/usr/local/bin/controller-net-sender`
is sufficient for protocol compatibility. To use the recommended queue size, also
set `BT_QUEUE_SIZE=16` in `/etc/default/controller-forwarder` and restart
`controller-udevmon.service`.
